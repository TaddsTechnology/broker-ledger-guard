  ...
  const fetchBillItemsAndBroker = async () => {
    ...
    try {
      const [itemsRes, brokersRes] = await Promise.all([
        fetch(`http://localhost:3001/api/fo/bills/${bill.id}/items`),
      ...
      console.error("Error fetching FO broker bill data", err);
    }
    setLoading(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="space-y-1">
          <div className="flex items-center justify-between">
            <DialogTitle className="flex items-center gap-2">
              ...
              <span className="text-[#9333ea]">F&O Sub-Broker Bill - {bill.bill_number}</span>
            </DialogTitle>
        ...
        <div className="space-y-6">
          {/* Bill Header */}
          <div className="grid grid-cols-2 gap-4 p-4 bg-gradient-to-r from-purple-50 to-fuchsia-50 rounded-lg border border-purple-200">
            <div>
              <p className="text-xs text-muted-foreground">Main Broker</p>
              <p className="font-semibold text-[#9333ea]">{bill.broker_code}</p>
            ...
            <div className="text-right">
              ...
              <p className="mt-1 text-xs">
                Main Broker Bill Amount: <span className="font-semibold text-[#9333ea]">
          ...
          {/* Profit Summary */}
          <div className="p-6 rounded-lg border border-purple-200 bg-gradient-to-r from-purple-50 to-emerald-50 flex flex-col items-center justify-center text-center">
            <p className="text-sm font-semibold text-[#9333ea] uppercase tracking-wide">Your F&O Sub-Broker Profit</p>
            ...
            <p className="text-sm text-[#9333ea] mt-2">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {Object.entries(clientGroups).map(([client, clientItems]) => {
                ...
                return (
                  <div key={client} className="border border-purple-200 rounded-lg p-4">
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="font-semibold text-[#9333ea]">{client}</h4>
