...
interface FOLedgerBillsPrintViewProps {
...
export const FOLedgerBillsPrintView: React.FC<FOLedgerBillsPrintViewProps> = ({
  ledgerEntries,
  selectedPartyId,
  parties,
  selectedBrokerId,
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
      <head>
        <title>F&O Ledger Bills Print</title>
        <style>
          body { 
        ...
        <div class="header">
          ...
          <div class="header-info">
            ${selectedParty ? `Party: ${selectedParty.party_code} - ${selectedParty.name}` : 
              selectedBrokerId && selectedBrokerId !== "all" ? 'Broker Entries' : 'All Transactions'}
          </div>
          ...
          <tbody>
            ${ledgerEntries.map((entry, index) => `
              <tr>
                ...
                <td>
                  <div class="font-medium">${entry.party_code || 'N/A'}</div>
                  <div style="font-size: 10px; color: #666;">${entry.party_name || 'Transaction'}</div>
                </td>
