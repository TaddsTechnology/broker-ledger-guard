...
export default function FOSummaryModulePage() {
  ...
  const fetchSummary = async (partyCode: string = "") => {
    ...
    try {
      const url = partyCode 
        ? `${API_BASE}/api/summary/parties?module=fo&party_code=${encodeURIComponent(partyCode)}`
        : `${API_BASE}/api/summary/parties?module=fo`;
  ...
  return (
    <div className="flex-1 overflow-auto">
      <PageHeader
        title="F&O Party Ledger Summary"
        description="Party-wise debit, credit and closing balance from F&O ledger"
        action={
